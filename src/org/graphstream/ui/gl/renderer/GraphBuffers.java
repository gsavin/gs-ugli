	...
	implements Sink
{
	...
	protected static class DraftID2Index
		implements ID2Index
	{
		/**
		 * 
		...
		protected int getPosition( String id )
		{
			...
			for( int i = 0; i < max; i++ )
				if( data.get( i * 2 + ID ) == hash ) return i;
		...
		{
			for( int i = 0; i < max; i++ )
				if( data.get( i * 2 + INDEX ) == index ) return i;
	...
	void printBuffers( java.io.PrintStream out )
	{
		...
		for( int i = 0; i <= lastNodeIndex; i++ )
		{
			...
			out.printf("%d : (%f;%f;%f)%n", index, nodeVerticesF.get( 3 * index ), nodeVerticesF.get( 3 * index + 1 ), nodeVerticesF.get( 3 * index + 2 ) );
		}
	}
	...
	{
		nodeIndexes   = ByteBuffer.allocateDirect(maxNodes*Integer.SIZE).order( ByteOrder.nativeOrder() );
		nodeIndexesI  = nodeIndexes.asIntBuffer();
		nodeVertices  = ByteBuffer.allocateDirect(3*maxNodes*Float.SIZE).order( ByteOrder.nativeOrder() );
		...
		{
			nodeColors  = ByteBuffer.allocateDirect(4*maxNodes*Float.SIZE).order( ByteOrder.nativeOrder() );
			nodeColorsF = nodeColors.asFloatBuffer();
		}
		else
		{
			nodeColors  = ByteBuffer.allocateDirect(4*4*Float.SIZE).order( ByteOrder.nativeOrder() );
			nodeColorsF = nodeColors.asFloatBuffer();
		}
		...
		edges  = ByteBuffer.allocateDirect(2*maxEdges).order( ByteOrder.nativeOrder() );
		...
		long size = nodeIndexes.capacity() + nodeVertices.capacity() + edges.capacity() + nodeColors.capacity();
		String symbol = "o";
	...
	protected void checkNodeCoords( String nodeId, String attr, Object value )
	{
		...
		if( index != -1 )
		{
			...
			else System.err.printf("unknown coords type%n");
		...
		else System.err.printf("id not found%n");
	}
	...
	public void nodeAdded(String sourceId, long timeId, String nodeId)
	{
		if( lastNodeIndex >= nodeIndexesI.capacity() - 1 )
			throw new OutOfMemoryError( String .format(
					...
					"current value: %d nodes%n", nodeIndexesI.capacity() ) );
